<template>
  <div class="final">
    ...
    <div>
      ...
      <el-dialog title="终审-详情" :visible.sync="dialogFormVisible">
...
export default {
  ...
  methods: {
    ...
    pass (row) {
      request.post('/api/approve/end/pass', { appId: row.id, loanId: row.loan_id })
        .then(res => {
          this.getData()
          this.$message({
            type: 'success',
    ...
    refuse (row) {
      request.post('/api/approve/end/reject', { appId: row.id, loanId: row.loan_id })
        .then(res => {
          this.getData()
          this.$message({
            type: 'success',
