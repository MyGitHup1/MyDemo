...
export default {
  ...
  // 添加一个指令 只能绑定在form标签上面 使表单只读;
  directives: {
    'formOnlyRead': {
      inserted: function (el) {
        if (el.tagName === 'FORM') {
          el.querySelectorAll('input').forEach(o => { o.setAttribute('readOnly', ''); o.setAttribute('disabled', '') })
        } else {
          throw new Error('v-formOnlyRead指令只能绑定在form标签上')
        }
      }
    }
  }
